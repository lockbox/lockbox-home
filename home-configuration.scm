...
(define (directory->config-alist source-dir target-dir)
  ...
  (let* ((this-file (current-source-location))
         (this-dir (dirname (assoc-ref this-file
                                       'filename)))
...
(home-environment
  ...
  (services
   (list (service home-xdg-user-directories-service-type
         ...
         (simple-service 'gnupg-files home-files-service-type
                         `((".gnupg/gpg.conf" ,(local-file "configs/gpg.conf"))
                           (".gnupg/scdaemon.conf" ,(local-file
                                                     "configs/scdaemon.conf"))))
         (simple-service 'ssh-config home-files-service-type
                         `((".ssh/config" ,(local-file "configs/ssh-config"))))
         ;; TODO: get scripts working
         (simple-service 'scripts home-files-service-type
                         `((".local/bin/reconfigure" ,(local-file
                                                       "scripts/reconfigure"))))
         (simple-service 'config-files
                         home-xdg-configuration-files-service-type
                         (append `(("dunst/dunstrc" ,(local-file
                                                      "configs/dunstrc"))
                                   ("i3/config" ,(local-file
                                                  "configs/i3-config"))
                                   ("i3status/config" ,(local-file
                                                        "configs/i3status-config"))
                                   ("i3status-rust/config.toml" ,(local-file
                                                                  "configs/i3status-rs.toml"))
                                   ("xdg-desktop-portal/sway-portals.conf" ,(local-file
                                                                             "configs/xdg-desktop-portal-sway-portals.conf"))
                                   ("sway/config" ,(local-file
                                                    "configs/sway.config"))
                                   ("polybar/config.ini" ,(local-file
                                                           "configs/polybar.ini"))
                                   ("polybar/launch" ,(local-file
                                                       "configs/launch_polybar.sh"))
                                   ("picom/picom.conf" ,(local-file
                                                         "configs/picom.conf"))
                                   ("rofi/config.rasi" ,(local-file
                                                         "configs/rofi-config.rasi"))
                                   ("rofi/theme.rasi" ,(local-file
                                                        "configs/rofi-themes/black.rasi"))
                                   ("starship.toml" ,(local-file
                                                      "configs/starship.toml"))
                                   ("ripgreprc" ,(local-file
                                                  "configs/ripgreprc"))
                                   ("git/config" ,(local-file
                                                   "configs/git-config"))
                                   ("git/ignore" ,(local-file
                                                   "configs/git-ignore"))
                                   ("wezterm/wezterm.lua" ,(local-file
                                                            "configs/wezterm.lua"))
                                   ("zellij/config.kdl" ,(local-file
                                                          "configs/zellij-config.kdl")))
                                 ;; Doom Emacs configuration - recursively copy entire directory
                                 (directory->config-alist "configs/doom"
                                                          "doom")))
         (simple-service 'extra-channels-service
                         ...
                         (list (channel
                               ...
                               (channel
                                 (name 'small-guix)
                                 (url
                                  "https://codeberg.org/fishinthecalculator/small-guix.git")
                                 (introduction
                                  (make-channel-introduction
         ...
         (service home-bash-service-type
                  (home-bash-configuration
                    ...
                    (aliases '(("docker-compose" . "docker compose")
                               ...
                               ("e" . "emacsclient -a=''")))
                    (bashrc (list (local-file "profiles/bash/bashrc" "bashrc")))
                    (bash-profile (list (local-file
                                         "profiles/bash/bash_profile"
                                         "bash_profile")))
                    (bash-logout (list (local-file "profiles/bash/bash_logout"
                                                   "bash_logout"))))))))
